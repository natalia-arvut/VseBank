...
export default function Reviews() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) { setError('Нет активной сессии'); return }
    ...
    if (imageFile) {
      ...
      if (uploadErr) {
        setLoading(false)
        setError(`Не удалось загрузить фото: ${uploadErr.message}`)
        return
      }
    ...
    setInfo('Спасибо за отзыв! Он появится в общем списке после модерации.')
    loadReviews()
  }
      ...
      <div className="p-6 md:p-10">
        ...
        <div className="xl:grid xl:grid-cols-2 xl:gap-6 xl:items-start">
          ...
          <div>
            {publishedReviews.length === 0 ? (
              <div className="glass-card p-12 text-center rounded-2xl">
                ...
                <p className="font-sans text-ink-700 leading-relaxed mb-1">Пока никто не оставил отзыв.</p>
                <p className="font-sans text-ink-500 text-sm">Будь первым — заполни форму справа.</p>
              </div>
            ) : (
            ...
            {myPendingReviews.length > 0 && (
              <div className="mt-8">
                <div className="tag text-sm mb-3">Твои отзывы на модерации</div>
            ...
            <div className="mt-10 pt-6 border-t border-gold-300/30">
              <p className="font-sans text-xs text-ink-500 leading-relaxed italic">
                От автора: на платформе публикуются только проверенные и доброжелательные отзывы. Мы оставляем за собой право не публиковать сообщения, содержащие негатив, оскорбления или нарушающие дух нашего сообщества.
              </p>
            </div>
          </div>
          ...
          <div className="glass-card p-6 md:p-8 rounded-2xl">
            <div className="tag text-sm mb-2">Поделись своей историей</div>
            <h2 className="font-serif text-2xl text-ink-900 mb-2">Твой отзыв</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Расскажи, как Вселенский Банк изменил твою жизнь, какие переводы получил, как изменилось твоё отношение к деньгам..."
                  value={text}
...
function ReviewCard({ review, pending }: { review: Review; pending?: boolean }) {
  return (
    <div className={`glass-card p-5 rounded-2xl ${pending ? 'opacity-70' : ''}`}>
      ...
      {pending && (
        <div className="text-xs font-sans text-gold-700 mb-2 italic">На модерации</div>
      )}
