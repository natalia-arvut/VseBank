...
export default function CabinetLayout({
  ...
  return (
    <div className="min-h-screen bg-cream-100 flex">
      ...
      {/* Мобильная навигация снизу — все 6 пунктов */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-stone-800 flex justify-around items-center py-2 z-20 border-t border-white/10 safe-bottom">
        {NAV_ITEMS.map(item => {
          const active = location.pathname === item.path
          return (
            <button
              key={item.path}
              onClick={() => navigate(item.path)}
              className={`flex flex-col items-center justify-center gap-0.5 px-1 py-1 flex-1 min-w-0 ${
                active ? 'text-gold-400' : 'text-stone-400'
              }`}
            >
              <span className="text-base leading-none">{item.icon}</span>
              <span className="font-sans text-[9px] leading-tight tracking-tight truncate w-full text-center">
