...
│   │   └── requisites.ts     # CRUD сохранённых реквизитов
│   ├── pages/
│   │   ├── Landing.tsx       # лендинг
│   │   ├── Login.tsx         # вход
...
| `public.requisites` | id (UUID), user_id (FK), label, recipient_name, bank_name, iban, bic | SELECT/INSERT/UPDATE/DELETE — own |
...
3. `003_requisites.sql` — requisites
4. `004_transfer_email_trigger.sql` — pg_net + триггер вызова Edge Function
...
4. ✅ Реквизиты в БД (миграция с localStorage автоматическая при первом заходе)
